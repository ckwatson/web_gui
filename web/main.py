...
# one-liners:
all_files_in = lambda mypath, end='': [os.path.splitext(os.path.basename(f))[0] for f in os.listdir(
    mypath) if os.path.isfile(os.path.join(mypath, f)) and not f.startswith('.') and f.endswith(end)]
    ...
    try:
        ...
        #    return # TODO: proper handle the conflict!
            ...
            with open("puzzles/" + data['puzzle'] + ".puz") as json_file:
                ...
                logger.info(
                    '    (0) Successfully loaded Puzzle Data from file!')
            ...
            for reagent, PERsToggles in puzzleData['reagentPERs'].items():
                ...
                preEqulElemRxns = [puzzleData['coefficient_array'][i]
                                   for i in range(num_rxn) if PERsToggles[i]]
                if preEqulElemRxns == []:
                    logger.info('        For the reagent #' + str(reagentID) +
                                ' "' + reagent + '", no pre-equilibration needed.')
                    ...
                    logger.info('        For the reagent #' + str(reagentID) + ' "' + reagent +
                    ...
                    if_uninvolvedSpecies = np.all(
                        preEqulElemRxns == False, axis=0)
                    ...
                    for speciesID in range(num_mol):
                        ...
                        if if_uninvolvedSpecies[speciesID]:
                            ...
                            displacement += 1
                    logger.info(
                        '            speciesList         : ' + str(speciesList))
                    logger.info('            if_uninvolvedSpecies: ' +
                                str(if_uninvolvedSpecies))
                    # \n',preEqulElemRxns)
                    logger.info(
                        '            This should mask the previous matrix into: //omitted//')
                    reagent_speciesList = [speciesList[i] for i in range(
                        num_mol) if not if_uninvolvedSpecies[i]]
                reagent_num_rxn = len(preEqulElemRxns)
                reagent_num_mol = len(reagent_speciesList)
                logger.info('            About pre-equilibration:')
                logger.info('                Elem. Rxn.s used for pre-equilibration (a total of ' +
                            str(reagent_num_rxn) + '): \n                    ' + str(preEqulElemRxns))
                logger.info('                which involves ' +
                            str(reagent_num_mol) + ' species: ' + str(reagent_speciesList))
                reagentsDict.append((reagent, reaction_mechanism_class.reaction_mechanism(
                    reagent_num_rxn, reagent_num_mol, reagent_speciesList, preEqulElemRxns, energy_dict)))
            #         - - - - - - - - - - - - - - -
            this_puzzle = puzzle_class.puzzle(
                num_rxn, num_mol, speciesList, elemRxns, energy_dict, reagent_dictionary=reagentsDict, Ea=Ea)
            ...
            r_temps = [reactant['temperature']
                       for reactant in data['conditions']]
            m_concs = [0.0] * num_mol
            #         - - - - - - - - - - - - - - -
            ...
            for each_rxn_proposed in data['reactions']:
                ...
                for each_slot in each_rxn_proposed:
                    if each_slot is not '':
                        coefficient_line_proposed[speciesList.index(
                            each_slot)] += 1
            ...
            this_solution = solution_class.solution(
                num_rxn_proposed, num_mol, speciesList, coefficient_array_proposed, energy_dict)
            ...
            trueModel_fileName = path_condition + \
                "plotData_t_" + str(temperature)
            if os.path.isfile(trueModel_fileName + '_.dat') and if_useCache:
                ...
                written_true_data = fileIO.load_modelData(
                    trueModel_fileName + '_.dat')
            else:
                ...
                logger.info("            Got result in a type of " +
                            str(type(written_true_data)) + '.')
            logger.info('         (b) User Model then:')
            # anticipate the file name where the true model's data is stored
            userModel_fileName = path_solution + \
                "plotData_t_" + str(temperature)
...
def save():
    ...
    try:
        ...
        # now convert:
        coefficient_dict = {species: i for i,
                            species in enumerate(data['speciesNames'])}
        coefficient_array = []
